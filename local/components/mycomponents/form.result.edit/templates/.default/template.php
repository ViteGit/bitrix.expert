...
<table class="form-info-table data-table">
    ...
    <tr>
        ...
        <td>
            [<a href='/bitrix/admin/form_edit.php?lang=<?= LANGUAGE_ID ?>&ID=<?= $arResult["WEB_FORM_ID"] ?>'><?= $arResult["WEB_FORM_ID"] ?></a>]&nbsp;(<?= $arResult["WEB_FORM_NAME"] ?>
            )&nbsp;<?= $arResult["FORM_TITLE"] ?></td>
    </tr>
    <?
    }
    ?>
        ...
        if ($arResult["isStatisticIncluded"] == "Y") {
            ...
            <tr>
                ...
                <td>[<a title="<?= GetMessage("FORM_GUEST_ALT") ?>"
                        href="/bitrix/admin/guest_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arResult["RESULT_STAT_GUEST_ID"] ?>&find_id_exact_match=Y&set_filter=Y"><?= $arResult["RESULT_STAT_GUEST_ID"] ?></a>]
                </td>
            </tr>
            <tr>
                <td><b><?= GetMessage("FORM_SESSION") ?></b></td>
                <td>
                    [<a href="/bitrix/admin/session_list.php?lang=<?= LANGUAGE_ID ?>&find_id=<?= $arResult["RESULT_STAT_SESSION_ID"] ?>&find_id_exact_match=Y&set_filter=Y"><?= $arResult["RESULT_STAT_SESSION_ID"] ?></a>]
                </td>
            </tr>
            <?
        }
...
<table class="form-table data-table">
    ...
    foreach ($arResult["QUESTIONS"] as $FIELD_SID => $arQuestion) {
        ...
        <? elseif ($FIELD_SID == 'SIMPLE_QUESTION_237'): ?>
